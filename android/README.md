...
서명된 빌드는 `*-release.apk`로 출력됩니다. `android-build-manifest.json`이 버전·ABI·서명 종류·크기·SHA-256을 기록합니다. `package.json`의 버전이 Android `versionName`의 기준이며, 1.0.11의 `versionCode`는 10011입니다.
...
```sh
node android/scripts/verify-apk.mjs dist/release/aidot-mini-1.0.11-android-x86_64-debug.apk
```
