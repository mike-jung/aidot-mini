...
function renderSettings(){
  ...
  for(const name of['workspace','host','port','https','certFile','keyFile','language','logLevel']){
...
$('#settings-form').addEventListener('submit',e=>{e.preventDefault();busy(e.currentTarget,async()=>{
  const value={};for(const name of['workspace','host','port','https','certFile','keyFile','language','logLevel']){const el=$(`[name="${name}"]`);if(!el.disabled)value[name]=name==='https'?el.checked:name==='port'?Number(el.value):el.value;}
