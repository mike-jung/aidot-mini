...
New examples use `@aidot/...` imports on both hosts, independent of the workspace location. Existing `../../src/...` code remains supported by mini; Express requires the original relative layout for that legacy syntax. Do not assume all historical imports are portable.
...
- The selected workspace migrations and named queries are the executable schema reference.
