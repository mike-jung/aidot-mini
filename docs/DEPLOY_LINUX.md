# Linux servers and robot boards — 1.0.13
...
## Current 1.0.13 release packages
...
```sh
tar -xzf aidot-mini-1.0.13-linux-x64-minimal.tar.gz
cd aidot-mini-1.0.13-linux-x64-minimal
...
The robot artifact contains `ros-src/` source workspaces, not a preinstalled ROS distribution or hardware drivers. Build those bridges with a compatible ROS environment and verify the actual robot board before deployment. Cross-building ARM64 on x64 does not prove ARM64 execution. Check the current [release validation record](RELEASE_1.0.13_KO.md).
...
```sh
tar -xzf aidot-mini-1.0.13-linux-x64.tar.gz
cd aidot-mini-1.0.13-linux-x64
...
```sh
sudo dpkg -i aidot-mini_1.0.13_amd64.deb
