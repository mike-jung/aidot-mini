...
import { androidToolchain } from './build-tools.mjs';
...
async function main() {
  ...
  if (args.includes('--help')) {
    console.log('dist:android [--abi arm64-v8a,x86_64] [--offline] [--prepare-only] [--debug-only]\nMissing npm dependencies are installed automatically. APK builds require Python 3, JDK 17+ and Android SDK platform 36.');
    return;
  }
  ...
  ensureBuildDependencies({ root, offline: args.includes('--offline') });
  const toolchain = args.includes('--prepare-only') ? null : androidToolchain(root);
  ...
  if (!args.includes('--prepare-only')) {
    ...
    run(toolchain.java, ['-Dorg.gradle.appname=gradlew', '-classpath', path.join(root, 'android/gradle/wrapper/gradle-wrapper.jar'), 'org.gradle.wrapper.GradleWrapperMain', ...gradleArgs], { cwd: path.join(root, 'android') });
