...
if (positionals.length !== 1 || !['original', 'auth', 'simple'].includes(values.example)) {
  throw new Error('Usage: npm run workspace:init -- /absolute/path [--empty | --example original|auth|simple]');
}
if (values.empty && values.example !== 'original' && values.example !== 'auth') throw new Error('--empty and --example simple cannot be combined');
const target = path.resolve(positionals[0]);
const source = path.join(ROOT, values.example === 'auth' ? 'examples/note-auth-workspace' : 'workspace');
...
if (!values.empty) console.log(values.example === 'auth' ? 'All Note CRUD routes require authentication.' : 'All Note CRUD routes are public. Administrator console access requires ID/password.');
